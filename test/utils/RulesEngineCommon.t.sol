...
contract RulesEngineCommon is DiamondMine, Test {
    ...
    function setupRuleWithForeignCallWithArrayToAnotherForeignCall(
        ...
        fc.signature = bytes4(keccak256(bytes("testSigWithArraySetInternallyOneArg(uint256)")));
    ...
    function setupRuleWithForeignCallWithArrayToAnotherForeignCallNoParameter(
        EffectTypes _effectType,
        bool isPositive
    ) public ifDeploymentTestsEnabled endWithStopPrank resetsGlobalVariables returns (uint256 policyId) {
        uint256[] memory policyIds = new uint256[](1);
        ...
        ForeignCall memory fc;
        fc.foreignCallAddress = address(foreignCall);
        fc.signature = bytes4(keccak256(bytes("testSigWithArraySetInternallyNoArg()")));
        fc.parameterTypes = new ParamTypes[](1);
        fc.parameterTypes[0] = ParamTypes.VOID;
        fc.encodedIndices = new ForeignCallEncodedIndex[](1);
        fc.encodedIndices[0].index = 1;
        fc.encodedIndices[0].eType = EncodedIndexType.ENCODED_VALUES;
        fc.foreignCallIndex = 0;
